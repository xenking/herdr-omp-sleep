...
- The frozen view is not a screenshot and not a description: the whole conversation is re-rendered by **omp's own transcript components**, in **your** theme and settings — the same `initTheme(symbolPreset, colorBlindMode, theme.dark, theme.light)` call `omp gallery` makes, so real colours, message bubbles, tool-call cards and usage footers. omp's last live frame is appended underneath it verbatim, so the bottom of the screen is exactly what you left.
...
- `omp-hist` opens that same render for a **live** session at any time, with the same keys. Compaction shrinks what the model sees and takes the TUI's scrollback with it; the `.jsonl` on disk keeps everything, and this is how you read it.
...
**`omp-frozen`** builds two views per width — folded and unfolded, each with the baked frame appended — and opens both in one `less`, at the end of the conversation: ENTER wakes, `q`/Ctrl-C exits to the shell, `o`/`O` switch folded ⇄ full, `r` re-renders at the width the pane has now. The renders are cached per session *and per width* (omp's components pre-wrap every row, so a resized pane needs a new render, not a folded old one) and invalidated when the session, the renderer, or the baked frame is newer than the view. Width comes from `stty size </dev/tty`, not `tput cols`: `tput` asks through stdout, which is a pipe inside a command substitution, so it answers 80 every time — which is what silently rendered this machine's entire parked fleet at 80 columns.
